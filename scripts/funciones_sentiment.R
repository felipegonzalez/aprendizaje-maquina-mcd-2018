...
convertir_lista <- function(df, vocabulario, stem = FALSE){
  vocabulario_1 <- arrange(vocabulario, desc(frec))
  vocabulario_1$id_palabra <- 2:(nrow(vocabulario_1)+1)
  df_filt <- df %>% # unnest_tokens(palabra, texto, token = 'words') %>%
    ungroup %>%
    arrange(id) %>%
    full_join(vocabulario_1) %>% 
    select(-frec) %>%
    inner_join(vocabulario_1 %>% select(id_palabra), by = c('id_palabra')) %>%
    filter(!is.na(id))
