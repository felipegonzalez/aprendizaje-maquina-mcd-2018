...
calc_error <- function(mod,  datos_p, y_name = "SalePrice"){
    ...
    print(qplot(preds, datos_p[[y_name]], alpha=0.5) + geom_abline() +
        annotate("text", x = 0, y= 300000, label = paste0("rmse ", round(error))))
    error
}
...
Ahora otras variables: el tamaño del área habitable y número 
de cuartos excluyendo el sótano:
...
```{r}
mod_3 <- lm(SalePrice ~  OverallQual + I(OverallQual^2) + GrLivArea + TotalBsmtSF +
...
```{r}
coef(mod_5)
bsmt_ind  <- str_detect(names(coef(mod_5)), "BsmtQual")
coef(mod_5)[bsmt_ind] %>% sort
```
...
En muchos problemas, es natural transformar variables numéricas a logarítmicas. 
...
---
Este es un
modelo multiplicativo, pues tomando exponencial de ambos lados, obtenemos:

$$y = B_0\prod x_j^{b_j}$$
Por ejemplo, si cambiamos $x_j -> x_j + \Delta x = x_j (1+ \frac{\Delta x}{x_j})$, el cambio porcentual en $y$ es
$$ \frac{y+\Delta y}{y} = \left (1 + \frac{\Delta x}{x_j}\right )^{b_j}$$
Y si $\frac{\Delta x}{x_j}$ es chica, entonces aproximadamente
...
cambios relativamente chicos en $x_j$.
