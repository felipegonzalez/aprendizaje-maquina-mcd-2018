...
**Observación**:
Aunque parece estar "volteado" el filtro con
respecto a la señal, la interpretación correcta es como sigue: 

- Al tiempo 0, la contribución a $y_0$ de $x_0$ es $x_0f_0$.
- Al tiempo 1, la contribución a $y_1$ de $x_0$ es 
$x_0f_1$ 
- Al tiempo 2, la contribución a $y_2$ de $x_0$ es 
$x_0f_2$ 

De modo que las $f_0$, $f_1$, $f_2$ etc. nos dice cuál es la
contribución de la señal en el momento actual, hace una
unidad de tiempo, hace dos unidades de tiempo, etc.
...
\begin{equation}
y_{a,b} = \sum_{s,t=-\infty}^{\infty} x_{s,t} f_{a-s,b-t}
\end{equation}
...
if(!usar_cache){
    ...
    model_2 %>%
      ...
      layer_max_pooling_2d(pool_size = c(2, 2), name = 'conv_1') %>% 
      layer_dropout(rate = 0.25) %>% 
      layer_conv_2d(filters = 4, kernel_size = c(3,3), 
                    activation = 'relu',
                    padding = 'same',
                    kernel_regularizer = regularizer_l2(0.01)) %>% 
      layer_max_pooling_2d(pool_size = c(2, 2), name = 'conv_2') %>% 
      layer_dropout(rate = 0.25) %>% 
      layer_flatten() %>% 
...
capa_list <- lapply(1:8, function(i){
    ...
    mutate(y = (pixel-1) %% 5, x = (pixel-1) %/% 5) %>%
     group_by(unidad)
capa_list
graficar_pesos <- function(capa_list){
    ggplot(capa_list, aes(x=x, y=-y)) + 
...
Nota que estos son detectores de formas geométricas simples (diagonales, rectas). Podemos ver las activaciones de la primera capa para algunos dígitos:
...
graf_activaciones <- function(activaciones, ind){
...
graf_activaciones(activaciones_1, 4)
graf_activaciones(activaciones_1, 5)
graf_activaciones(activaciones_1, 15)
graf_activaciones(activaciones_1, 8)
graf_activaciones(activaciones_1, 33)
```
...
la primera y la cuarta unidad de salida:
```{r}
ggplot(capa_out %>% filter(unidad %in% c(1,4)), 
    aes(x = x, y = -y)) + geom_tile(aes(fill = (val))) + 
  facet_grid(origen ~ unidad) + coord_equal()+
  scale_fill_gradient2(low = "red", mid='gray70',high = "black") + 
  theme(strip.background = element_blank(), strip.text = element_blank())    
```

El primer filtro produce valores altos de activación cuando, en
la salida de la primera capa de convolución:

- Las unidades 5 y 7 están apagadas (no detecta recta horizontal).
- La unidad 2, 4, 6 y 8 están encendidas (detecta rectas verticales).
- La unidad 1 tiene un gradiente creciente de arriba a abajo.

Considerando los pesos de la capa anterior, esta capa produce
valores altos de activación cuando:

- La imagen no tiene rectas horizontales.
- La imagen tiene rectas verticales.

El filtro 4 es similar, excepto que no penaliza rectas verticales.
...
```{r}
require(gridExtra)
grid.arrange(graf_activaciones(activaciones_1, 4), 
             graf_activaciones(activaciones_2, 4), ncol=2)
grid.arrange(graf_activaciones(activaciones_1, 5), 
             graf_activaciones(activaciones_2, 5), ncol=2)
grid.arrange(graf_activaciones(activaciones_1, 15), 
             graf_activaciones(activaciones_2, 15), ncol=2)
grid.arrange(graf_activaciones(activaciones_1, 33), 
             graf_activaciones(activaciones_2, 33), ncol=2)
```
