...
x_train_3 <- ( entrena_3 %>% select(-digito) %>% as.matrix + 1 ) / 2
...
history_1 <- modelo_sin_reg %>% fit(x_train_3, y_train_3, verbose=0,
...
Los pesos aprendidos en la capa oculta parecen ruidosos, e incluso parece
que hay unidades que no tienen mucho propósito:
...
history_2 <- modelo_dropout %>% fit(x_train_3, y_train_3, verbose = 0,
  epochs = 800, batch_size = 256, validation_split = 0.2)
...
El desempeño es mejor, y  los patrones que aprendió
en el capa oculta parecen ser más útiles:
...
Ahora veamos cómo se activan distintas unidades con diferentes entradas.
En este caso, calculamos las activaciones de las 30 unidades ocultas para
5 dígitos del conjunto de entrenamiento (una matriz de 30 por 5):

```{r}
indices <- c(2, 10, 16, 29, 30)
...
Y podemos graficar sólo aquellas unidades que están activas
en cada caso:
...
graf_pesos(pesos[, dense_t[ ,5] > 0], mostrar_facets = TRUE)
